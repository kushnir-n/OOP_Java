...
public abstract class Hero implements Stepable, Comparable<Hero> {
    ...
    public String attack(Hero currentHero, int id, ArrayList<Hero> heroes) {
        ...
        if (power == 0) {
            ...
            if (heroes.get(id).getHealth() <= 0) {
                ...
                MyView.MatrixCoordination[heroes.get(id).getPosition().getY()][heroes.get(id).getPosition().getX()] = Color.MAGENTA_BOLD + "*" + Color.RESET;
                heroes.remove(id);
    ...
    private void moveHero(Position currentPosition, int dy, int dx, Hero hero) {
        MyView.MatrixCoordination[currentPosition.getY()][currentPosition.getX()] = Color.MAGENTA_BOLD + "*" + Color.RESET;
        MyView.MatrixCoordination[currentPosition.getY() + dy][currentPosition.getX() + dx] = hero.getTeamColor() + hero.getInfo() + Color.RESET;
    }
                ...
                if (Math.abs(dx) > Math.abs(dy)) {
                    if (dx > 0) {
                        if (canMove(myTeam, new Position(currentPosition.getX() - 1, currentPosition.getY()))) {
                            moveHero(currentPosition, 0, -1, this);
                    ...
                    if (dy > 0) {
                        if (canMove(myTeam, new Position(currentPosition.getX(), currentPosition.getY() - 1))) {
                            moveHero(currentPosition, -1, 0, this);
    ...
    public int compareTo(Hero to) {
        int compareInitiative = to.getInitiative();

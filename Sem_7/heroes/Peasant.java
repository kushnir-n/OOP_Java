...
public class Peasant extends Hero { //крестьянин
    ...
    public String digPotatoes() {
        return "Potatoes, so many potatoes!!!";
    }

    private boolean checkTeam(ArrayList<Hero> heroes) {
        int cnt = 0;
        for (Hero hero : heroes) {
            if (hero instanceof Crossbower || hero instanceof Sniper) {
                cnt++;
            }
        }
        return cnt > 0;
    ...
    public String step(Position currentPosition, ArrayList<Hero> heroes, ArrayList<Hero> myTeam) {
        if (checkTeam(myTeam)){
            ...
            System.out.printf("\"К такому меня жизнь не учила\", сказал %s и пошёл в рукопашку...\n", getName());
